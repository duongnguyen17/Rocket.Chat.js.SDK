...
class Client implements IClient {
  ...
  get(url: string, data: any, options?: any): Promise<any> {
    return fetch(
      `${this.host}/api/v1/${encodeURI(url)}?${this.getParams(data)}`,
      {
        method: "GET",
...
export default class Api extends EventEmitter {
  ...
  request = async (
    ...
    try {
      ...
      switch (method) {
        case "GET":
          result = await this.client.get(endpoint, data, options);
          break;
        case "PUT":
          result = await this.client.put(endpoint, data, options);
          break;
        case "DELETE":
          result = await this.client.delete(endpoint, data, options);
          break;
        default:
        case "POST":
          result = await this.client.post(endpoint, data, options);
          break;
      }
      if (!result)
      ...
      this.logger &&
        this.logger.debug(
          `[API] ${method} ${endpoint} result ${result.status}`
        );
      ...
      this.logger &&
        this.logger.error(
          `[API] POST error(${endpoint}): ${JSON.stringify(err)}`
        );
      throw err;
    }
  };
  /** Do a POST request to an API endpoint. */
  post: IAPIRequest = (endpoint, data, auth, ignore, options = {}) =>
    this.request("POST", endpoint, data, auth, ignore, options);

  /** Do a GET request to an API endpoint. */
  get: IAPIRequest = (endpoint, data, auth, ignore, options = {}) =>
    this.request("GET", endpoint, data, auth, ignore, options);

  /** Do a PUT request to an API endpoint. */
  put: IAPIRequest = (endpoint, data, auth, ignore, options = {}) =>
    this.request("PUT", endpoint, data, auth, ignore, options);

  /** Do a DELETE request to an API endpoint. */
  del: IAPIRequest = (endpoint, data, auth, ignore, options = {}) =>
    this.request("DELETE", endpoint, data, auth, ignore, options);
